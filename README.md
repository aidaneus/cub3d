...
#### Задача:
```
1. Игра должна находиться в отдельном окне, которое может сворачиваться,
   передвигаться с помощью курсора, закрываться на крестик.
2. Стены должны иметь текстуры, которые отличаются в зависимости от
   распожения(север, юг, восток, запад).
3. Программа должна иметь возможность устанавливать два различных цвета
   для пола и потолка.
4. Клавиши со стрелками влево и вправо должны позволять игроку смотреть
   влево и вправо соответственно.
5. Программа должна принимать в качестве первого аргумента файл с
   расширением .cub - собранная сцена игры по выбранной карте.
6. Карта должна состоять только из 6 возможных символов: 0 для пустого
...
10. Игра должна подстраиваться под любой тип карты, которая
...
#### The Task
```
1. The game must be in a separate window that can be minimized, moved
   with the cursor, closed with a cross.
2. Walls should have textures that differ depending on the
   location(north, south, east, west).
3. The program must be able to set two different colors for the floor
   and ceiling.
4. The left and right arrow keys should allow the player to look left
   and right respectively.
5. The program must take as a first argument a scene description file
   with the .cub extension - the assembled game scene for the selected map.
6. The map must be composed of only 6 possible characters: 0 for an
